...
const ResidentDashboard: React.FC<ResidentDashboardProps> = ({ 
  ...
  const myReservations = reservations.filter(r => r.userId === user.id).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  const myAnnouncements = announcements.filter(a => !a.readBy.includes(user.id));
